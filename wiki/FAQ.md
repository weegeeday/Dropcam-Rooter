...
Yes. Now that you have root shell access, you can run the local Ambarella camera tools. The camera filesystem contains local video encoder and server binaries (such as `test_encode` or `rtsp_server` libraries). You can configure `ie_auto.sh` to start a local RTSP server to stream video directly into home automation hubs (e.g. Home Assistant, Frigate) completely offline.
