...
Yes. Now that you have root shell access, you can run a comaptible rtsp streamer. (Il be working on some software to have the full capabilities of the dropcam in home assistant later on.)  You can configure `ie_auto.sh` to start a local RTSP server to stream video directly into home automation hubs (e.g. Home Assistant, Frigate) completely offline. Here's a quick command that at least gives you an RTSP stream, its not the best, but its better than nothing for testing. `/usr/local/bin/mediaserver -a -f /etc/dropcam/encode.cfg &`
